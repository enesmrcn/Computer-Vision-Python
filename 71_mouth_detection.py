...
def process(image) :
    ...
    for (x, y, width, height) in faces :
        ...
        for (ex, ey, ew, eh) in faces :
            ...
            cv.putText(roi_face_color, 'eye', (ex - 20, ey - 10), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

        smile = smile_cascade.detectMultiScale(roi_face_gray, minSize=(80, 80))

        for (sx, sy, sw, sh) in smile :
            cv.rectangle(roi_face_color, (sx, sy), (sx + sw, sy + sh), (0, 255, 0), 3, cv.LINE_AA)
            cv.putText(roi_face_color, 'mouth', (sx - 20, sy - 10), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
